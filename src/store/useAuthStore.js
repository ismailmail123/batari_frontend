    ...
    persist(
        (set, get) => ({
            ...
            verify: async(data) => {
                set({ isSigningUp: true });
                try {
                    const res = await axios.post("http://localhost:3001/api/auth/verify-email", data);
                    set({ authUser: res.data });
                    localStorage.setItem("authUser", JSON.stringify(res.data)); // Simpan data user di local storage
                } catch (error) {
            ...
            logout: async() => {
                try {
                    await axios.post("http://localhost:3001/api/auth/logout"); // Panggil API logout
            ...
            updateProfile: async(data) => {
                ...
                try {
                    const res = await axios.put("http://localhost:3001/api/auth/update-profile", data);
                    set({ authUser: res.data });
                    localStorage.setItem("authUser", JSON.stringify(res.data)); // Simpan data user di local storage
