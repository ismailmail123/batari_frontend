...
const FloatingKeyboardToggleHome = ({ isKeyboardEnabled, onToggleKeyboard }) => {
  return (
    <button
      onClick={onToggleKeyboard}
      className={`fixed right-6 bottom-24 z-40 group relative p-3 rounded-full shadow-2xl focus:outline-none focus:ring-4 transition-all duration-300 transform hover:scale-110 ${
        isKeyboardEnabled 
          ? 'bg-green-500 text-white hover:bg-green-600 focus:ring-green-300' 
          : 'bg-gray-500 text-white hover:bg-gray-600 focus:ring-gray-300'
      }`}
      title={`${isKeyboardEnabled ? 'Nonaktifkan' : 'Aktifkan'} Keyboard Virtual`}
      style={{ width: '52px', height: '52px' }} // Atur ukuran tetap untuk bulat sempurna
    >
      <FaKeyboard className="w-6 h-6" />
      
      <div className="absolute right-full mr-3 top-1/2 transform -translate-y-1/2 px-3 py-2 bg-gray-800 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none">
        {isKeyboardEnabled ? 'Nonaktifkan' : 'Aktifkan'} Keyboard Virtual
        <div className="absolute left-full top-1/2 transform -translate-y-1/2 border-8 border-transparent border-l-gray-800"></div>
      </div>
    </button>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {authUser?.user?.role === "admin" && showScreensaver ? (
        ...
        <div className="flex flex-col min-h-screen bg-gray-50">
          {/* Header */}
          <header className="bg-blue-600 text-white shadow-lg relative flex-shrink-0">
          ...
          {authUser.user?.role === "admin" && (
            <>
              ...
              {/* Footer */}
              <footer className="bg-blue-600 text-white mt-auto py-4 flex-shrink-0">
