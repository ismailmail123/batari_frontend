...
const PengunjungDetail = () => {
  ...
  // Handle Print Label Titipan
  const handlePrintLabel = useReactToPrint({
    content: () => labelTitipanRef.current,
  });
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        {/* Action Buttons - DITAMBAHKAN TOMBOL LABEL TITIPAN */}
          ...
          {authUser.user.role === "admin" && (
            <>
              {pengunjungByCode.status === "Tidak Valid" && (
                <button
                  onClick={handleVerify}
                  className="bg-blue-600 text-black px-4 py-2 rounded text-bold hover:bg-blue-700"
                >
                  verifikasi
                </button>
              )}
              ...
              <button
                onClick={() => setShowPreview(true)}
                className="bg-green-600 text-black px-4 py-2 rounded text-bold hover:bg-green-700"
              >
                Cetak
              </button>
              <button
                onClick={handleDownloadPDF}
                className="bg-purple-600 text-black px-4 py-2 rounded text-bold hover:bg-purple-700"
              >
                Export PDF
              </button>
              {/* TOMBOL CETAK LABEL TITIPAN */}
              {pengunjungByCode.barang_titipan?.length > 0 && (
                <>
                  <button
                    onClick={() => setShowLabelPreview(true)}
                    className="bg-orange-600 text-black px-4 py-2 rounded text-bold hover:bg-orange-700"
                  >
                    Cetak Label Titipan
                  </button>
                  <button
                    onClick={handleDownloadLabelPDF}
                    className="bg-red-600 text-black px-4 py-2 rounded text-bold hover:bg-red-700"
                  >
                    Export Label PDF
                  </button>
                </>
            ...
            <>
              ...
              {pengunjungByCode.barang_titipan?.length > 0 && (
                <>
                  <button
                    onClick={() => setShowLabelPreview(true)}
                    className="bg-orange-600 text-black px-4 py-2 rounded hover:bg-orange-700"
                  >
                    Cetak Label Titipan
                  </button>
                  <button
